...
@Component({
          ...
          <div class="form-wrapper">
            ...
            <form *ngIf="!mfaRequired" [formGroup]="loginForm" (ngSubmit)="onSubmit()" class="auth-form">
              <div class="form-group">
                ...
                <!-- Validation messages using modern control flow -->
  ...
  styles: [`
    ...
    .logo-box {
      width: 44px; height: 44px; background: #1e1e2d; border-radius: 12px;
      position: relative; display: flex; align-items: center; justify-content: center;
      border: 1px solid rgba(245, 158, 11, 0.3);
    }
    .logo-glow { position: absolute; inset: -2px; background: linear-gradient(135deg, #f59e0b, transparent); border-radius: 13px; opacity: 0.5; }
    .logo-icon-auth { color: white; font-size: 1.5rem; z-index: 1; }
    .brand-text-group .brand-name { font-size: 1.25rem; font-weight: 800; margin: 0; letter-spacing: 0.05em; }
    ...
    .batch-dots { display: flex; gap: 0.4rem; }
    .dot { width: 24px; height: 24px; background: rgba(255,255,255,0.1); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.7rem; font-weight: 800; }
    .dot.active { background: #f59e0b; color: #000; }
    .batch-label { font-size: 0.75rem; font-weight: 800; color: #f59e0b; }
    .progress-bar { height: 6px; background: rgba(255,255,255,0.1); border-radius: 10px; margin: 1.5rem 0; overflow: hidden; }
    .progress-fill { height: 100%; background: #f59e0b; border-radius: 10px; }
    .batch-footer { display: flex; justify-content: space-between; align-items: center; font-size: 0.75rem; }
    .footer-label { opacity: 0.4; font-weight: 600; }
    .footer-value { font-weight: 700; font-size: 0.85rem; }

    /* Right Panel */
    .right-panel { flex: 1; padding: 3.5rem; display: flex; flex-direction: column; justify-content: center; position: relative; }
    ...
    .btn-text-link { background: none; border: none; color: #2563eb; font-weight: 600; padding: 0; cursor: pointer; font-size: inherit; }
    .btn-text-link:hover { text-decoration: underline; }

    .form-group { margin-bottom: 1.5rem; position: relative; }
    .input-label { display: block; font-size: 0.7rem; font-weight: 800; color: #9ca3af; margin-bottom: 0.6rem; }
    .input-container { position: relative; display: flex; align-items: center; }
    .input-container i { position: absolute; left: 1rem; color: #9ca3af; font-size: 1.1rem; }
    .auth-input { width: 100%; background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 10px; padding: 0.8rem 1rem 0.8rem 2.8rem; font-size: 0.95rem; transition: all 0.2s; }
    .auth-input:focus { outline: none; border-color: #f59e0b; background: white; box-shadow: 0 0 0 4px rgba(245, 158, 11, 0.1); }
    .auth-input.input-error { border-color: #dc2626; }
    .error-message { display: block; font-size: 0.8rem; color: #dc2626; margin-top: 0.35rem; }

    .text-center { text-align: center; }
    .mfa-input { letter-spacing: 0.5rem; font-size: 1.5rem; font-weight: 800; padding: 1rem; }
    .mfa-hint { font-size: 0.75rem; color: #9ca3af; margin-top: 0.5rem; font-weight: 500; }

    .forgot-btn { position: absolute; right: 0; bottom: -1.4rem; background: none; border: none; font-size: 0.7rem; font-weight: 800; color: #9ca3af; cursor: pointer; }
    .submit-btn { width: 100%; background: #2563eb; color: white; border: none; border-radius: 12px; padding: 1.1rem; font-weight: 700; font-size: 1rem; display: flex; align-items: center; justify-content: center; gap: 0.5rem; margin-top: 2.5rem; cursor: pointer; box-shadow: 0 10px 15px -3px rgba(37, 99, 235, 0.25); transition: all 0.2s; }
    ...
    .divider span { position: relative; background: white; padding: 0 1rem; font-size: 0.65rem; font-weight: 800; color: #9ca3af; letter-spacing: 0.1em; }
    .social-grid { display: flex; gap: 1rem; }
    .social-btn { flex: 1; background: white; border: 1px solid #e5e7eb; border-radius: 12px; padding: 0.75rem; font-weight: 600; font-size: 0.9rem; display: flex; align-items: center; justify-content: center; gap: 0.75rem; cursor: pointer; }
    .color-linkedin { color: #0077b5; }
    .color-microsoft { color: #5e5e5e; }
    .copyright { position: absolute; bottom: 2rem; left: 0; width: 100%; text-align: center; font-size: 0.7rem; font-weight: 800; color: #d1d5db; }
    ...
    @media (max-width: 900px) {
      ...
      .left-panel, .right-panel { padding: 2.5rem; }
    }
  `]
})
